...
- Fixed an issue where the Import from Google Classroom sidebar showed no courses in the dropdown, even when the teacher had active Google Classroom courses.
...
- Improved reliability of automatic imports and reports. In some cases, automatic features could repeatedly fail silently due to permission or account access issues — these will now be detected and stopped automatically to prevent ongoing errors.

### Version 6.49
**Released: April 8, 2026**
- Fixed an issue where importing grades from Google Classroom could fail mid-import for large classes due to a temporary Google API error. The import will now automatically retry and complete successfully.
...
- General improvements to the import process.
...
- Fixed license validation issues.
